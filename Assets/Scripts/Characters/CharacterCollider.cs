...
public class CharacterCollider : MonoBehaviour
{
    ...
    /// <summary>
    /// 슬라이드해야 피할 수 있었던건지 체크.
    /// </summary>
    public bool ShouldHaveSlided { get; private set; }

    protected Vector3 startColliderSize;

    protected void Start()
    {
        m_Collider = GetComponent<BoxCollider>();
        startColliderSize = m_Collider.size;
    ...
    //추가한 기능
    /// <summary>
    /// 플레이어 콜라이더에서 방향값에 장애물이 있는지 boxcast로 검출합니다. <br/>
    /// 검사는 다음과 같이 작동합니다: <br/>
    /// 기준점: 콜라이더 월드 기준 중심점 + 콜라이더 y크기 * 방향값.y <br/>
    /// 검사 넓이 : 콜라이더 크기와 동일함. 
    /// </summary>
    /// <param name="direction">방향값</param>
    /// <returns>검출 여부</returns>
    /// 사용 용도: 플레이어가 점프 혹은 슬라이드로 피하게 된 장애물이 있는지 검사
    bool CheckObstacle(Vector3 direction, Vector3 size)
    {
        RaycastHit[] results = new RaycastHit[5];
        Collider[] colresults = new Collider[5];
        int count = Physics.BoxCastNonAlloc(transform.position + m_Collider.center, m_Collider.size / 2, direction, results, Quaternion.identity, 3, obstacleMask, QueryTriggerInteraction.Collide);

        /*int count = Physics.OverlapBoxNonAlloc(
			transform.position + m_Collider.center + direction * m_Collider.size.y,
			size / 2,
			colresults,
			Quaternion.identity,
			obstacleMask,
    ...
    protected void Update()
    {
        ...
        ShouldHaveSlided = CheckObstacle(Vector3.up, Vector3.Scale(startColliderSize, k_SlidingColliderScale));

        ShouldHaveJumped = CheckObstacle(Vector3.down, startColliderSize);
    }
