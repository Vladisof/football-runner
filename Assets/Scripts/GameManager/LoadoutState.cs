...
namespace GameManager
{
	public class LoadoutState : SwState
	{
		...
		public TextMeshProUGUI accesoryNameDisplay;
		...
		Consumable.Consumables.ConsumableType m_PowerupToUse = Consumable.Consumables.ConsumableType.NONE;
		...
		public override void Enter()
		{
			...
			if(m_PowerupToUse != Consumable.Consumables.ConsumableType.NONE)
			{
				...
				if (!PlayerData.instance.consumables.ContainsKey(m_PowerupToUse) || PlayerData.instance.consumables[m_PowerupToUse] == 0)
					m_PowerupToUse = Consumable.Consumables.ConsumableType.NONE;
			}
		...
		public override void Exit(SwState to)
		{
			...
			if (gs != null)
			{
				...
				if (m_PowerupToUse != Consumable.Consumables.ConsumableType.NONE)
				{
					...
					Consumable.Consumables inv = Instantiate(ConsumablesDatabase.GetConsumbale(m_PowerupToUse));
					inv.gameObject.SetActive(false);
					gs.trackManager.CharactersController.inventory = inv;
				}
			}
		}
		...
		public override void Tick()
		{
			...
			{
				bool interactable = ThemeDatabase.loaded && CharactersDatabase.loaded;
				if(interactable)
				{
		...
		public IEnumerator PopulateCharacters()
		{
			...
			if (!m_IsLoadingCharacter)
			{
				...
				while (newChar == null)
				{
					Characters.Characters c = CharactersDatabase.GetCharacter(PlayerData.instance.characters[PlayerData.instance.usedCharacter]);
		...
		void SetupAccessory()
		{
			Characters.Characters c = m_Character.GetComponent<Characters.Characters>();
			c.SetupAccessor(PlayerData.instance.usedAccessory);
		...
		void PopulatePowerup()
		{
			...
			{
				Consumable.Consumables c = ConsumablesDatabase.GetConsumbale(m_PowerupToUse);
		...
		public void ChangeConsumable(int dir)
		{
			...
			do
			{
				...
				if(m_UsedPowerupIndex >= (int)Consumable.Consumables.ConsumableType.MAX_COUNT)
				{
					m_UsedPowerupIndex = 0; 
				}
				else if(m_UsedPowerupIndex < 0)
				{
					m_UsedPowerupIndex = (int)Consumable.Consumables.ConsumableType.MAX_COUNT - 1;
				}

				int count = 0;
				if(PlayerData.instance.consumables.TryGetValue((Consumable.Consumables.ConsumableType)m_UsedPowerupIndex, out count) && count > 0)
				{
					found = true;
				}
			...
			m_PowerupToUse = (Consumable.Consumables.ConsumableType)m_UsedPowerupIndex;
			PopulatePowerup();
		}
